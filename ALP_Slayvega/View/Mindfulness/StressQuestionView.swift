    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ForEach(0..<viewModel.questions.count, id: \.self) { index in
                    Rectangle()
                        .fill(index <= currentQuestionIndex ? Color(red: 1.0, green: 0.56, blue: 0.427) : Color.gray.opacity(0.3))
                        .frame(height: 3)
                }
            }
            .padding(.horizontal, 20)
            ...
            if currentQuestionIndex < viewModel.questions.count {
                ...
                VStack(spacing: 40) {
                    VStack(spacing: 20) {
                        Text("How well does the following statement describe you?")
                            .font(.title3)
                            .foregroundColor(.gray)
        ...
        .alert(isPresented: $showResult) {
            Alert(
                ...
                message: Text("Your stress level is: \(viewModel.stressLevel)"),
                primaryButton: .default(Text("View Details")) {
